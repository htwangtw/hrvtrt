...
Although BOLD signal is, theoretically, heavily confounded by physiology signals, researchers still found functional relevance of physiology signal after removing relevant confunds.
...
From the converging evidence above, we designed this analysis to understand the relevance of physiology signal in resting state functional connectivity and explore the individual differences of its cognitive and psychiatric relevance.
- RQ1 - Understand the relevance of physiology signal in fMRI resting state functional connectivity.
- RQ2 - Assess the test-retest reliability of the impact of physiology data on functional connectivity.
...
To test RQ1, we will determine whether fMRI resting state functional connectivity, using the data from session 1, is significantly correlated with any of: ongoing heart rate, ongoing respiration, or heart-rate variability.
We will explore methods that were used in the past literature, such as GLM ([Kassinopoulos et al., 2019]) that yield activation maps that show voxels most relevant to physiology, as well as novel methods developed in this paper (see Statistical Method).
We will also explore novel connectivity-based gradients to understand the impact of physiology on the whole brain.
...
The main author has published a study using the cross section resting state fMRI data (TR=2.5), MRI questionnaire (MRI-Q) and cognitive domain assessments ([Wang et. al., 2018]).
...
#### Physiology data
The pulse oximetry and respiratory volume data will be prerocessed according to [Kassinopoulos et al., 2019] 2.2.1. Preprocessing of physiological recordings.
The preprocessing procedure will be developed using simulated test data before applying it to real dataset.

#### Exclusion criteria
After fMRI data preprocessing, we will exclude subjects with more than 50% of high motion volumes in the timeseries (> 450 volumes).
Motion is defined by a framewise displacement above 0.5 mm, calculated by `fMRIprep`.
...
Phenotype missing data will be imputed with either most frequent occurence or variable mean, depending on if data is categorical(frequent occurrence), ordinal(frequent occurrence), or scale (mean).
...
`RETROICOR` parameters will be created as closely as possible to the implementation in [Chen et al., 2020].

#### Physiological response functions
For the physiological response relevant to the functional connectivity analysis, we adopt a strategy introduced by [Kassinopoulos et al., 2019]. The original MATLAB implementation can be found [here](https://github.com/mkassinopoulos/PRF_estimation).
...
For each ROI, we calculate an interaction term of signal and the physiological data, and then correlate this interaction with the remaining raw signal extracted from ROIs.
This step will result in an asymmetric functional connectivity matrix for each subject.

We will quantify the difference with a pure connectivity based gradient with R2 to describe the impact of physiology.

We will also compute a group label the cortical diffusion embedding map of this asymmetrical connectivity for exploration purpose.
...
As the main aim of the analysis is to understand test-retest reliability, we adopt measures described in the past literature. For both univariate and multivariate test-retest reliability measure, we will use intraclass correlation coefficient (ICC), see: [Noble et al., 2020].
ICC can be interpreted as follows: <0.4 poor; 0.4–0.59 fair; 0.60–0.74 good; > 0.74 excellent ([Cicchetti and Sparrow 1981])
